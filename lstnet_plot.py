...
def AutoCorrelationPlot(Data, init):
    ...
    if init.save_plot is not None:
        log.debug(
            "Saving autocorrelation plot to: %s",
            f"{init.save_plot}_autocorrelation.png",
        )
        fig.savefig(f"{init.save_plot}_autocorrelation.png")


def PlotHistory(history, metrics, init):
    if history is None or metrics is None or init is None:
        return
    ...
    for m in metrics:
        ...
        if key in history.keys():
            ...
            plt.ylabel(m.title())
            plt.xlabel("Epochs")
            plt.title(f"Validation {m.title()}")
            i = i + 1
    ...
    if init.save_plot is not None:
        log.debug(
            "Saving training history plot to: %s",
            f"{init.save_plot}_training.png",
        )
        fig.savefig(f"{init.save_plot}_training.png")
...
def PlotPrediction(Data, init, trainPredict, validPredict, testPredict):
    ...
    if init.save_plot is not None:
        log.debug("Saving prediction plot to: %s", f"{init.save_plot}_prediction.png")
        fig.savefig(f"{init.save_plot}_prediction.png")
